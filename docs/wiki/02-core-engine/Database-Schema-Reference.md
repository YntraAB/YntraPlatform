# Complete 18-Table Database Schema & Migrations Dictionary
...
## 7. Offline Client Incremental Migration Protocol

When native iOS/Android devices or web apps update after extended offline periods (potentially skipping multiple version releases), `yntra-core` executes an incremental migration loop before opening database read queries:
...
1. **Version Detection**: `yntra-core` compares `PRAGMA user_version` against the target schema version compiled into the current engine binary.
2. **Sequential Execution**: If `user_version < TARGET_VERSION`, migrations execute sequentially inside explicit transactions (`BEGIN IMMEDIATE;` ... `PRAGMA user_version = N; COMMIT;`).
3. **Rollback Safety**: If any migration step fails, the local transaction is rolled back, preserving the database state, and an error alert is sent to `DatabaseObserver`.
