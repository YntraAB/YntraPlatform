                    ...
                    div { class: "flex h-16 shrink-0 items-center justify-between border-b border-border/50 px-8 bg-sidebar/50",
                        div { class: "flex items-center gap-5",
                            ...
                            select {
                                ...
                                onchange: move |e| {
                                    time_filter_status.set(e.value());
                                    selected_time_reports.set(Vec::new());
                                },
                                option { value: "all", "Alla rapporter ({filtered_reports.len()})" }
                                option { value: "pending_attest", "Väntar attest ({filtered_reports.iter().filter(|r| r.status == \"pending_attest\").count()})" }
                        ...
                        div { class: "flex items-center gap-5",
                            ...
                            div { class: "relative hidden sm:block",
                                ...
                                input {
                                    class: "yntra-input text-xs h-9 pl-9 rounded-lg border-none bg-white/[0.02] focus:outline-none focus:ring-1 focus:ring-primary/20",
                                    placeholder: t("timereports-search-shifts-placeholder", &props.locale),
                                    value: "{time_search_query}",
                                    oninput: move |e| time_search_query.set(e.value()),
                                }
                            }
