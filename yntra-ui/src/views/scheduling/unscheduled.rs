...
pub struct UnscheduledBucketProps {
    ...
    pub unscheduled_jobs: Vec<yntra_core::JobTicket>,
    pub dragged_job_id: Signal<Option<String>>,
    pub is_moving_company: bool,
...
pub fn UnscheduledBucket(props: UnscheduledBucketProps) -> Element {
    ...
    let mut dragged_job_id = props.dragged_job_id;
        ...
        div {
            ...
            if props.is_moving_company {
                div { class: "p-4 border-b border-border flex items-center justify-between bg-background/50",
                    h3 { class: "font-semibold text-xs uppercase tracking-widest text-muted-foreground flex items-center gap-2 m-0 select-none",
                        components::LucideIcon { name: "box", class: "h-4 w-4 text-primary" }
                        if props.locale == "sv" { "Oschemalagda uppdrag" } else { "Unscheduled Jobs" }
                    }
                    span { class: "bg-primary/10 text-primary text-[10px] px-2 py-0.5 rounded-full font-bold border border-primary/20",
                        "{props.unscheduled_jobs.len()}"
                    }
                }
            } else {
                div { class: "p-4 border-b border-border flex items-center justify-between bg-background/50",
            ...
            if props.is_moving_company {
                div { class: "flex-1 overflow-y-auto p-3 flex flex-col gap-2 scrollbar-dark",
                    if props.unscheduled_jobs.is_empty() {
                        div { class: "text-center py-12 px-4 border border-dashed border-border/50 rounded-xl bg-muted/10",
                            components::LucideIcon { name: "truck", class: "h-8 w-8 mx-auto text-muted-foreground/30 mb-2" }
                            div { class: "text-muted-foreground text-xs italic",
                                if props.locale == "sv" { "Inga oschemalagda uppdrag" } else { "No unscheduled jobs" }
                            }
                        }
                    } else {
                        for item in props.unscheduled_jobs.iter().map(|job| {
                            let job_id = job.id.clone();
                            let job_title = job.title.clone();
                            let priority = job.priority.clone();
                            let location = if let (Some(org), Some(dst)) = (&job.origin_address, &job.destination_address) {
                                format!("{} ➔ {}", org, dst)
                            } else {
                                job.location_address.clone()
                            };
                            let priority_color = match priority.as_str() {
                                "high" => "hsl(0, 84.2%, 60.2%)",
                                "medium" => "hsl(37.9, 90.2%, 50.2%)",
                                _ => "hsl(215.4, 16.3%, 56.9%)",
                            };
                            let is_dragged = Some(job_id.clone()) == *dragged_job_id.read();
                            let drag_style = if is_dragged { "opacity: 0.5; transform: scale(0.95);" } else { "" };
                            (job_id, job_title, priority, location, priority_color, is_dragged, drag_style)
                        }) {
                            div {
                                key: "{item.0}",
                                draggable: is_editing,
                                ondragstart: move |_| {
                                    dragged_job_id.set(Some(item.0.clone()));
                                },
                                ondragend: move |_| {
                                ...
                                style: "border-left: 4px solid {item.4}; {item.6}",

                                div { class: "text-sm font-semibold text-foreground truncate",
                                    "{item.1}"
                                }
                                div { class: "text-[10px] text-muted-foreground truncate mt-1 flex items-center gap-1",
                                    components::LucideIcon { name: "map-pin", size: "10" }
                                    "{item.3}"
                                }
                                div {
                                    class: "text-[9px] font-bold uppercase mt-1.5 tracking-wider px-1.5 py-0.5 rounded w-max",
                                    style: "background: {item.4}15; color: {item.4};",
                                    "{item.2}"
                                }
                            }
                        }
