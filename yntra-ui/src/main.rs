...
use locales::get_system_locale;
use state::AppState;
use utils::DioxusDbObserver;

use utils::get_supabase_user_email;
...
fn App() -> Element {
    ...
    let active_user = users.read().as_ref().and_then(|u_list| u_list.iter().find(|u| u.id == *active_user_id.read()).cloned()).unwrap_or_else(|| WorkspaceUser {
        id: String::new(),
        workspace_id: None,
        email: String::new(),
    ...
    let workspace_val = workspace.read().clone().unwrap_or_else(|| Workspace {
        id: "workspace-1".to_string(),
        name: "Yntra Operations Ltd".to_string(),
        modules_active: "{\"messaging\":true,\"scheduling\":true,\"notes\":true,\"time\":true,\"assistance\":true,\"directory\":true,\"reporting\":true}".to_string(),
        settings: "{}".to_string(),
    ...
    let users_data = users.read().clone().unwrap_or_default();
    let teams_data = teams.read().clone().unwrap_or_default();
    let notes_data = notes.read().clone().unwrap_or_default();
    let messages_data = messages.read().clone().unwrap_or_default();
    ...
    rsx! {
        ...
        if !*logged_in.read() {
            views::LoginView {
                ...
                on_desktop_oauth: handle_desktop_oauth,
            }
