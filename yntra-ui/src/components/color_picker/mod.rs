...
// ========================================================
// Backward Compatibility / String Hex Picker Wrapper
// ========================================================

#[derive(Props, Clone, PartialEq)]
pub struct HexColorPickerProps {
    pub value: String,
...
pub fn HexColorPicker(props: HexColorPickerProps) -> Element {
    ...
    let on_color_change = Callback::new(move |hsv: Hsv<encoding::Srgb, f64>| {
        let rgb: Color = Srgb::<f64>::from_color(hsv).into_format();
        let hex = format!("#{rgb:X}");
        onchange.call(hex);
    });

    rsx! {
        ColorPicker {
            color: ReadSignal::new(color_sig),
            on_color_change: on_color_change,
            disabled: ReadSignal::new(Signal::new(props.disabled)),
            open: ReadSignal::new(Signal::new(None)),
            default_open: false,
