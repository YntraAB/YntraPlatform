...
/// The props for the [`TabRoot`] component.
#[derive(Props, Clone, PartialEq)]
pub struct TabRootProps {
...
#[component]
pub fn TabRoot(props: TabRootProps) -> Element {
...
#[component]
pub fn Tabs(props: LegacyTabsProps) -> Element {
    rsx! {
        div {
            class: Styles::dx_tabs_list,
            ...
            for tab in props.tabs.iter() {
                {
                    let is_active = tab.value == props.active_tab;
                    ...
                    rsx! {
                        button {
                            ...
                            "data-state": "{state}",
                            onclick: move |_| props.onchange.call(tab_val.clone()),
                            if let Some(icon) = icon_name {
                                crate::components::LucideIcon { name: icon, class: "h-4 w-4 mr-1.5" }
                            }
                            span { "{label}" }
                        }
                    }
