...
pub fn get_database() -> &'static libsql::Database {
    DATABASE.get_or_init(|| {
        block_on(async {
            ...
            setup_schema(&conn).await.expect("Failed to initialize database schema");
...
impl DbConnection {
    pub async fn execute<P: libsql::params::IntoParams + Send>(&self, sql: &str, params: P) -> Result<u64, YntraError> {
        self.inner.as_ref().unwrap().execute(sql, params).await
            .map_err(|e| YntraError::DbError(e.to_string()))
    }

    pub async fn execute_batch(&self, sql: &str) -> Result<(), YntraError> {
        self.inner.as_ref().unwrap().execute_batch(sql).await
            .map(|_| ()).map_err(|e| YntraError::DbError(e.to_string()))
    }
...
async fn setup_schema(conn: &DbConnection) -> Result<(), YntraError> {
    conn.execute(
    ...
    // JobTicket dual-address routing and access migrations
    ...
    // Moving Template: Inventory and Quotation tables
    ...
    // School & Education Template tables
    let _ = conn.execute_batch(
    ...
    // Migrations for student-user identity linking and parent associations
...
async fn seed_mock_data(conn: &DbConnection) {
    ...
    let mut admin_email = std::env::var("ADMIN_EMAIL").unwrap_or_default();
    ...
    // Ensure all current blocks exist, and defunct 'school' block is deleted
    let _ = conn.execute_batch(
        "
