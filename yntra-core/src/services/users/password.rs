...
pub async fn verify_email_password(
    ...
    if let Some(row) = rows.next().await? {
        ...
        if let Some(ref ws) = ws_id {
            ...
            if prefs_val.get("encrypted_workspace_key").is_none() {
                ...
                if getrandom::fill(&mut ws_key).is_ok() {
                    ...
                    if let Ok(enc_key) = enc_key_res {
                        prefs_val.as_object_mut().unwrap().insert(
                            "encrypted_workspace_key".to_string(),
                            serde_json::Value::String(enc_key),
                        );
            ...
            if let Some(enc_key) = prefs_val
                .get("encrypted_workspace_key")
                .and_then(|v| v.as_str())
            {
                #[cfg(not(target_arch = "wasm32"))]
                let pwd = zeroizing_password.to_string();
...
pub async fn set_user_password(
    ...
    if auth.role != "platform_admin" && target_ws_id.as_ref() != Some(&auth.workspace_id) {
        return Err(YntraError::AuthError(
            "Access denied: target user is not in your workspace".to_string(),
        ));
    }
