...
pub async fn calculate_eligible_labor_cost(
    ...
    let labor_portion = if pricing_model == "hourly" {
        ...
        let default_crew_size =
            crate::services::workspaces::get_setting_f64(settings_json, "moving_default_crew_size");
...
pub async fn calculate_customer_annual_rut_used(
    ...
    let mut rows = stmt
        .query(crate::params![workspace_id, customer_id, &year_prefix])
        .await?;
    ...
    while let Some(row) = quote_rows.next().await? {
        ...
        let crane_hoist_fee = if requires_crane_hoist {
        ...
        let eligible_labor =
            calculate_eligible_labor_cost(conn, &job_id, base_price, &settings_json)
                .await
                .unwrap_or(base_price * 0.70);
...
#[uniffi::export]
pub fn validate_customer_personal_number_for_rut(
    personal_number: &str,
...
pub async fn generate_move_invoice(
    ...
    let is_rut_active = (target_region == "SE"
        || settings_json
    ...
    let (rut_deduction, tax_authority_amount, customer_amount) = if is_rut_active {
        ...
        let valid_pnum = if let Some(ref pnum_str) = raw_pnum {
            let decrypted = if pnum_str.starts_with("enc:") || pnum_str.len() > 30 {
                crate::infra::crypto::decrypt_field(pnum_str, &ws_id)
                    .unwrap_or_else(|_| pnum_str.clone())
            } else {
                pnum_str.clone()
            };
            use chrono::Datelike;
        ...
        let used_rut =
            calculate_customer_annual_rut_used(&conn, &ws_id, &customer_id, &target_year, None)
                .await
                .unwrap_or(0.0);
        let annual_cap = crate::services::workspaces::get_setting_f64(
            &settings_json,
            "annual_rut_limit_per_person",
        );
    ...
    let mut inv_stmt = conn
        .prepare("SELECT id FROM move_invoices WHERE quote_id = ?1")
        .await?;
...
pub async fn pay_move_invoice(
    ...
    let mut stmt = conn
        .prepare("SELECT workspace_id, customer_id FROM move_invoices WHERE id = ?1")
        .await?;
...
pub async fn adjust_invoice_for_actuals(
    ...
    if !is_staff(&auth) {
        return Err(YntraError::AuthError(
            "Access denied: only staff can adjust invoices".to_string(),
        ));
    }
    ...
    let mut rows = stmt
        .query(crate::params![&invoice_id, &auth.workspace_id])
        .await?;
    let (quote_id, customer_id, invoice_date, due_date, current_status) =
        if let Some(row) = rows.next().await? {
            (
                row.get::<String>(0)?,
                row.get::<String>(1)?,
    ...
    let (job_ticket_id, base_price, distance_fee, stairs_surcharge, packing_supplies_fee) =
        if let Some(row) = quote_rows.next().await? {
            (
                row.get::<String>(0)?,
                row.get::<f64>(1)?,
                row.get::<f64>(2)?,
            ...
            return Err(YntraError::NotFoundError(
                "Corresponding quote not found".to_string(),
            ));
        };
    ...
    if pricing_model == "hourly" {
        if let Some(hrs) = actual_hours {
            ...
            let surcharge_fragile =
                crate::services::workspaces::get_setting_f64(&settings_json, "surcharge_fragile");
            let surcharge_server_rack = crate::services::workspaces::get_setting_f64(
                &settings_json,
                "surcharge_server_rack",
            );
            let surcharge_fitness_equipment = crate::services::workspaces::get_setting_f64(
                &settings_json,
                "surcharge_fitness_equipment",
            );
            let surcharge_marble_glass = crate::services::workspaces::get_setting_f64(
                &settings_json,
                "surcharge_marble_glass",
            );
            ...
            let min_hours = crate::services::workspaces::get_setting_f64(
                &settings_json,
    ...
    let subtotal =
        adjusted_base_price + distance_fee + stairs_surcharge + packing_supplies_fee + add_charges;
    ...
    let (rut_deduction, tax_authority_amount, customer_amount) = if is_rut_active {
        let eligible_labor = calculate_eligible_labor_cost(
            &conn,
            &job_ticket_id,
            adjusted_base_price,
            &settings_json,
        ...
        let non_deductible_stair_additions = long_carry_fee + crane_hoist_fee;
        let total_eligible_labor =
            (eligible_labor + stairs_surcharge - non_deductible_stair_additions).max(0.0);

        let raw_rut = 0.5 * total_eligible_labor;

        let target_year = if invoice_date.len() >= 4 {
            &invoice_date[0..4]
        } else {
            "2026"
        };
        ...
        .await
        .unwrap_or(0.0);
        let annual_cap = settings_json
            .get("annual_rut_limit_per_person")
            .and_then(|v| v.as_f64())
            .unwrap_or(75000.0);
        let remaining_cap = (annual_cap - used_rut).max(0.0);
        let rut = raw_rut.min(remaining_cap);
