...
pub async fn initiate_adyen_payment(
    ...
    let gateway_url = get_config_val("adyen_gateway_url", "ADYEN_GATEWAY_URL", &settings_json)
        .await
    ...
    let merchant_account = get_config_val(
    ...
    if let Some(gw_url) = gateway_url {
        ...
        let session: crate::models::AdyenPaymentSession = res
            .json()
            .await
            .map_err(|e| YntraError::NetworkError(e.to_string()))?;
        Ok(session)
        ...
        let url = if let Some(custom_url) =
            get_config_val("adyen_checkout_url", "ADYEN_CHECKOUT_URL", &settings_json).await
        {
            custom_url
        } else {
            let adyen_env =
                get_config_val("adyen_environment", "ADYEN_ENVIRONMENT", &settings_json)
                    .await
                    .unwrap_or_else(|| "test".to_string());
            if adyen_env.to_lowercase() == "live" || adyen_env.to_lowercase() == "production" {
                if let Some(prefix) =
                    get_config_val("adyen_live_prefix", "ADYEN_LIVE_PREFIX", &settings_json).await
                {
                    format!(
                        "https://{}-checkout-live.adyenpayments.com/checkout/v70/sessions",
                        prefix
                    )
                } else {
        ...
        let session_data = base64_encode(
            format!(
                "{{\"session_id\":\"{}\",\"amount\":{}}}",
                session_id, amount
            )
