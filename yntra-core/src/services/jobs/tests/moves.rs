...
use crate::services::jobs::{
    accept_move_quote, accept_move_quote_with_deposit, calculate_and_save_move_quote,
    confirm_quote_deposit_payment, create_job_ticket, get_move_quote, get_move_quote_revisions,
};
use crate::services::jobs::{
    add_job_packaging_item, get_job_packaging_items, remove_job_packaging_item,
};
use crate::services::jobs::{
    create_move_inventory_item, delete_move_inventory_item, get_move_inventory,
};
...
async fn test_move_operations() {
    ...
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE id = 'u-move-staff'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-move-test'", ())
        .await
        .unwrap();
}
...
async fn test_client_self_service_inventory_flow() {
    ...
    assert!(
        inv.iter()
            .any(|i| i.item_name == "Soffa" && i.quantity == 1)
    );
    assert!(
        inv.iter()
            .any(|i| i.item_name == "Kartong" && i.quantity == 10)
    );
    ...
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE id = 'u-client-inv'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-client-inv-test'", ())
        .await
        .unwrap();
}
...
async fn test_specialty_item_surcharges() {
    ...
    calculate_and_save_move_quote("u-spec-staff".to_string(), "job-spec-test".to_string())
        .await
        .unwrap();
    ...
    let quote = get_move_quote("u-spec-staff".to_string(), "job-spec-test".to_string())
        .await
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        calculate_item_specialty_surcharge(
            "Kontor".into(),
            "Skåp".into(),
            Some("Värdeskåp med kodlås / Tunglyft".into()),
            1500.0,
            2000.0,
            2500.0,
            500.0
    ...
    // 5. Clean up
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-spec-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM move_inventory WHERE workspace_id = 'ws-spec-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-spec-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-spec-test'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-spec-test'", ())
        .await
        .unwrap();
}
...
async fn test_packaging_inventory_system() {
    ...
    calculate_and_save_move_quote("u-pack-staff".to_string(), "job-pack-test".to_string())
        .await
        .unwrap();

    let quote1 = get_move_quote("u-pack-staff".to_string(), "job-pack-test".to_string())
        .await
        .unwrap()
        .unwrap();
    ...
    let items = get_job_packaging_items("u-pack-staff".to_string(), "job-pack-test".to_string())
        .await
        .unwrap();
    ...
    let quote2 = get_move_quote("u-pack-staff".to_string(), "job-pack-test".to_string())
        .await
        .unwrap()
        .unwrap();
    ...
    remove_job_packaging_item("u-pack-staff".to_string(), tape_item_id)
        .await
        .unwrap();

    let quote3 = get_move_quote("u-pack-staff".to_string(), "job-pack-test".to_string())
        .await
        .unwrap()
        .unwrap();
    ...
    // 6. Clean up
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-pack-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM move_inventory WHERE workspace_id = 'ws-pack-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM job_packaging_items WHERE job_ticket_id = 'job-pack-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-pack-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-pack-test'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-pack-test'", ())
        .await
        .unwrap();
}
...
async fn test_basement_carrying_surcharges() {
    ...
    use crate::services::jobs::moves::{
        calculate_access_and_stair_surcharge, calculate_access_and_stair_surcharge_with_multipliers,
    };
    // Default spiral staircase multiplier = 1.5 -> 2 floors * 300 kr * 1.5 = 900 kr
    assert_eq!(
        calculate_access_and_stair_surcharge(
            2,
            0,
            false,
            true,
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-basement-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM move_inventory WHERE workspace_id = 'ws-basement-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-basement-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM users WHERE workspace_id = 'ws-basement-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-basement-test'", ())
        .await
        .unwrap();
}
...
async fn test_dynamic_pricing_models() {
    ...
    calculate_and_save_move_quote("u-dyn-staff".to_string(), job.id.clone())
        .await
        .unwrap();

    let q = get_move_quote("u-dyn-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-dyn-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM move_inventory WHERE workspace_id = 'ws-dyn-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-dyn-test'",
        (),
    )
    .await
    .unwrap();
...
fn test_tariff_estimation_and_volume_conversions() {
    use crate::services::jobs::{
        calculate_packing_materials_tariff_estimate, convert_volume_to_tariff_weight,
    };
...
async fn test_warehouse_vault_sit_storage_workflow() {
    use crate::services::jobs::{
        assign_job_to_warehouse_vault, calculate_sit_recurring_billing_summary,
        get_job_warehouse_vaults, release_job_from_warehouse_vault,
    };
    ...
    let vaults = get_job_warehouse_vaults("u-sit-staff".to_string(), "job-sit-1".to_string())
        .await
        .unwrap();
    ...
    let summary =
        calculate_sit_recurring_billing_summary("u-sit-staff".to_string(), "job-sit-1".to_string())
            .await
            .unwrap();
    ...
    release_job_from_warehouse_vault("u-sit-staff".to_string(), vault.id)
        .await
        .unwrap();
    let vaults_after = get_job_warehouse_vaults("u-sit-staff".to_string(), "job-sit-1".to_string())
        .await
        .unwrap();
    assert_eq!(vaults_after[0].status, "released");

    conn.execute(
        "DELETE FROM warehouse_vaults WHERE workspace_id = 'ws-sit-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-sit-test'",
        (),
    )
    .await
    .ok();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-sit-test'", ())
        .await
        .ok();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-sit-test'", ())
        .await
        .ok();
}
...
async fn test_tip_distribution_and_mover_payroll_split() {
    use crate::services::jobs::{
        calculate_mover_job_payroll_split, create_job_ticket, distribute_job_customer_tip,
        get_job_tip_distribution,
    };
    ...
    let tip = distribute_job_customer_tip("u-pay-staff".to_string(), job.id.clone(), 600.0)
        .await
        .unwrap();
    assert_eq!(tip.crew_count, 2);
    assert_eq!(tip.tip_per_member_sek, 300.0);

    let tip_opt = get_job_tip_distribution("u-pay-staff".to_string(), job.id.clone())
        .await
        .unwrap();
    assert!(tip_opt.is_some());

    let driver_pay = calculate_mover_job_payroll_split(
        "u-pay-staff".to_string(),
        job.id.clone(),
        "u-pay-driver".to_string(),
        2.5,
        5.5,
        false,
    )
    ...
    let mover_pay = calculate_mover_job_payroll_split(
        "u-pay-staff".to_string(),
        job.id.clone(),
        "u-pay-mover".to_string(),
        0.0,
        10.0,
        true,
    )
    ...
    conn.execute(
        "DELETE FROM job_tips WHERE workspace_id = 'ws-pay-test'",
        (),
    )
    .await
    .ok();
    ...
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-pay-test'",
        (),
    )
    .await
    .ok();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-pay-test'", ())
        .await
        .ok();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-pay-test'", ())
        .await
        .ok();
}
...
async fn test_manual_override_dynamic_item_additions() {
    ...
    .await
    .unwrap();
    calculate_and_save_move_quote("u-ovr-staff".to_string(), job.id.clone())
        .await
        .unwrap();

    let q1 = get_move_quote("u-ovr-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    crate::services::jobs::update_move_quote_price_adjustments(
        "u-ovr-staff".to_string(),
        job.id.clone(),
        Some(3000.0),
        None,
    ...
    let q2 = get_move_quote("u-ovr-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    let q3 = get_move_quote("u-ovr-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    .await
    .unwrap();
    let q4 = get_move_quote("u-ovr-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-ovr-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM move_inventory WHERE workspace_id = 'ws-ovr-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-ovr-test'",
        (),
    )
    .await
    .ok();
...
async fn test_quote_revision_audit_trail_and_accepted_lock() {
    ...
    .await
    .unwrap();
    calculate_and_save_move_quote("u-rev-staff".to_string(), job.id.clone())
        .await
        .unwrap();

    let q1 = get_move_quote("u-rev-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    let revs = get_move_quote_revisions("u-rev-staff".to_string(), q1.id.clone())
        .await
        .unwrap();
    ...
    // 3. Accept quote
    accept_move_quote("u-rev-staff".to_string(), q1.id.clone())
        .await
        .unwrap();
    ...
    let guest_err = create_move_inventory_item(
        "u-client-guest".to_string(),
        job.id.clone(),
        "Möbler".to_string(),
        "Stol".to_string(),
        1,
    ...
    calculate_and_save_move_quote("u-rev-staff".to_string(), job.id.clone())
        .await
        .unwrap();
    let updated_quote = get_move_quote("u-rev-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quote_revisions WHERE workspace_id = 'ws-rev-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-rev-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM move_inventory WHERE workspace_id = 'ws-rev-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-rev-test'",
        (),
    )
    .await
    .ok();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-rev-test'", ())
        .await
        .ok();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-rev-test'", ())
        .await
        .ok();
}
...
async fn test_quote_deposit_flow_prevents_premature_accepted_lock() {
    ...
    .await
    .unwrap();
    calculate_and_save_move_quote("u-dep-staff".to_string(), job.id.clone())
        .await
        .unwrap();
    let q1 = get_move_quote("u-dep-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();

    // 1. Initiate deposit payment approval
    let dep_res = accept_move_quote_with_deposit(
        "u-dep-staff".to_string(),
        q1.id.clone(),
        "swish".to_string(),
    )
    ...
    let q_status_after: String = conn
        .query_row(
            "SELECT status FROM move_quotes WHERE id = ?1",
            crate::params![&q1.id],
            |r| r.get(0),
        ...
        .await
        .unwrap();
    let j_status_after: String = conn
        .query_row(
            "SELECT status FROM job_tickets WHERE id = ?1",
            crate::params![&job.id],
            |r| r.get(0),
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-dep-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM move_inventory WHERE workspace_id = 'ws-dep-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-dep-test'",
        (),
    )
    .await
    .ok();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-dep-test'", ())
        .await
        .ok();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-dep-test'", ())
        .await
        .ok();
}
...
async fn test_deposit_payment_link_desynchronization_prevention() {
    ...
    let job = create_job_ticket(
        "u-desync-staff".to_string(),
        "ws-dep-desync-test".to_string(),
        "Desync Job".to_string(),
        "Desc".to_string(),
        "Addr 1".to_string(),
        "medium".to_string(),
        None,
    ...
    .await
    .unwrap();
    calculate_and_save_move_quote("u-desync-staff".to_string(), job.id.clone())
        .await
        .unwrap();
    let q1 = get_move_quote("u-desync-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    let confirm_stale = confirm_quote_deposit_payment(
        "u-desync-staff".to_string(),
        q1.id.clone(),
        "ref-stale".to_string(),
    )
    ...
    assert!(
        confirm_stale
            .unwrap_err()
            .to_string()
            .contains("deposit payment link invalidated due to quote revisions")
    );

    // 4. Re-accepting quote with deposit produces new deposit amount based on updated total
    let dep_res2 = accept_move_quote_with_deposit(
        "u-desync-staff".to_string(),
        q1.id.clone(),
        "swish".to_string(),
    )
    ...
    let confirm_fresh = confirm_quote_deposit_payment(
        "u-desync-staff".to_string(),
        q1.id.clone(),
        "ref-fresh".to_string(),
    )
    ...
    let final_q_status: String = conn
        .query_row(
            "SELECT status FROM move_quotes WHERE id = ?1",
            crate::params![&q1.id],
            |r| r.get(0),
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-dep-desync-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM move_inventory WHERE workspace_id = 'ws-dep-desync-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-dep-desync-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM users WHERE workspace_id = 'ws-dep-desync-test'",
        (),
    )
    .await
    .ok();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-dep-desync-test'", ())
        .await
        .ok();
}
...
async fn test_dynamic_currency_labels_in_deposit_approval() {
    ...
    let job = create_job_ticket(
        "u-curr-staff".to_string(),
        "ws-curr-test".to_string(),
        "Euro Move".to_string(),
        "Desc".to_string(),
        "Berlin 1".to_string(),
        "medium".to_string(),
    ...
    .await
    .unwrap();
    calculate_and_save_move_quote("u-curr-staff".to_string(), job.id.clone())
        .await
        .unwrap();
    let q1 = get_move_quote("u-curr-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-curr-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM move_inventory WHERE workspace_id = 'ws-curr-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-curr-test'",
        (),
    )
    .await
    .ok();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-curr-test'", ())
        .await
        .ok();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-curr-test'", ())
        .await
        .ok();
}
...
async fn test_custom_domain_deposit_link_generation() {
    ...
    .await
    .unwrap();
    calculate_and_save_move_quote("u-cd-staff".to_string(), job.id.clone())
        .await
        .unwrap();
    let q1 = get_move_quote("u-cd-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-customdomain-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-customdomain-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM users WHERE workspace_id = 'ws-customdomain-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM workspaces WHERE id = 'ws-customdomain-test'",
        (),
    )
    .await
    .ok();
...
async fn test_per_job_distance_isolation() {
    ...
    calculate_and_save_move_quote("u-dist-staff".to_string(), job_a.id.clone())
        .await
        .unwrap();
    calculate_and_save_move_quote("u-dist-staff".to_string(), job_b.id.clone())
        .await
        .unwrap();

    let quote_a = get_move_quote("u-dist-staff".to_string(), job_a.id.clone())
        .await
        .unwrap()
        .unwrap();
    let quote_b = get_move_quote("u-dist-staff".to_string(), job_b.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-dist-iso-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-dist-iso-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM users WHERE workspace_id = 'ws-dist-iso-test'",
        (),
    )
    .await
    .ok();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-dist-iso-test'", ())
        .await
        .ok();
}
...
async fn test_depot_positioning_and_roundtrip_mileage() {
    ...
    conn.execute(
        "UPDATE job_tickets SET route_stops_json = ?1 WHERE id = ?2",
        crate::params![route_json, &job.id],
    )
    .await
    .unwrap();

    calculate_and_save_move_quote("u-depot-staff".to_string(), job.id.clone())
        .await
        .unwrap();

    let quote = get_move_quote("u-depot-staff".to_string(), job.id.clone())
        .await
        .unwrap()
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM move_quotes WHERE workspace_id = 'ws-depot-test'",
        (),
    )
    .await
    .ok();
    conn.execute(
        "DELETE FROM job_tickets WHERE workspace_id = 'ws-depot-test'",
        (),
    )
    .await
    .ok();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-depot-test'", ())
        .await
        .ok();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-depot-test'", ())
        .await
        .ok();
}
