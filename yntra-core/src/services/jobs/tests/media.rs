...
async fn test_offline_media_crdt_optimization() {
    ...
    let fetched = get_offline_media_pointer("u-media-crew".to_string(), pointer.hash_pointer.clone()).await.unwrap();
    ...
    let synced_count = sync_pending_offline_media_blobs("u-media-crew".to_string(), job.id.clone()).await.unwrap();
    assert_eq!(synced_count, 1);

    let fetched_synced = get_offline_media_pointer("u-media-crew".to_string(), pointer.hash_pointer.clone()).await.unwrap().unwrap();
    assert_eq!(fetched_synced.upload_status, "synced");

    // Cleanup
    conn.execute("DELETE FROM offline_media_blobs WHERE workspace_id = 'ws-media-test'", ()).await.unwrap();
    conn.execute("DELETE FROM job_tickets WHERE workspace_id = 'ws-media-test'", ()).await.unwrap();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-media-test'", ()).await.unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-media-test'", ()).await.unwrap();
}
