...
async fn send_external_notification_inner(
    ...
    let (customer_email, customer_phone, customer_name): (String, Option<String>, Option<String>) =
        conn.query_row(
    ...
    let plivo_from = settings
        .get("plivo_from_number")
        .and_then(|v| v.as_str())
        .unwrap_or("+18005550199");

    let whatsapp_id = settings
        .get("whatsapp_phone_number_id")
        .and_then(|v| v.as_str());
    let whatsapp_token = settings
        .get("whatsapp_access_token")
        .and_then(|v| v.as_str());

    let sendgrid_key = settings.get("sendgrid_api_key").and_then(|v| v.as_str());
    let sendgrid_from = settings
        .get("sendgrid_from_email")
        .and_then(|v| v.as_str())
        .unwrap_or("no-reply@yntra.se");
    ...
    if let (Some(auth_id), Some(auth_tok), Some(ref phone)) =
        (plivo_id, plivo_token, customer_phone.as_ref())
    {
    ...
    if let (Some(w_id), Some(w_token), Some(ref phone)) =
        (whatsapp_id, whatsapp_token, customer_phone.as_ref())
    {
        let client = reqwest::Client::new();
    ...
    if let (Some(sid), Some(token), Some(ref phone)) =
        (twilio_sid, twilio_token, customer_phone.as_ref())
    {
        let client = reqwest::Client::new();
        let url = format!(
...
pub async fn send_external_notification(
    ...
    let fut = send_external_notification_inner(
        requester_user_id,
        workspace_id,
        customer_id,
        trigger_type,
...
pub async fn get_customer_live_tracking_portal(
    ...
    Ok(crate::models::CustomerLiveTrackingPortal {
        ...
        destination_address: if to_addr.is_empty() {
            "Destinationsadress".to_string()
        } else {
            to_addr
        },
        origin_lat,
...
mod departure_eta_tests {
    ...
    async fn test_automated_sms_live_driver_eta_alerts_workflow() {
        ...
        let portal = get_customer_live_tracking_portal("job-eta-1".to_string())
            .await
            .unwrap();
        ...
        let sent = send_dispatch_departure_eta_sms(
            "u-eta-staff".to_string(),
            "job-eta-1".to_string(),
            "u-eta-cust".to_string(),
        )
        .await
        .unwrap();
        assert!(sent);
        ...
        let en_route = send_driver_en_route_alert(
            "u-eta-staff".to_string(),
            "job-eta-1".to_string(),
            "u-eta-cust".to_string(),
            Some(12),
        ...
        let eta_update = send_live_eta_update_alert(
            "u-eta-staff".to_string(),
            "job-eta-1".to_string(),
            "u-eta-cust".to_string(),
            8,
        ...
        let review_req = send_post_move_review_request(
            "u-eta-staff".to_string(),
            "job-eta-1".to_string(),
            "u-eta-cust".to_string(),
        )
        ...
        conn.execute(
            "DELETE FROM job_tickets WHERE workspace_id = 'ws-eta-test'",
            (),
        )
        .await
        .unwrap();
        conn.execute(
            "DELETE FROM vehicles WHERE workspace_id = 'ws-eta-test'",
            (),
        )
        .await
        .unwrap();
        conn.execute("DELETE FROM users WHERE workspace_id = 'ws-eta-test'", ())
            .await
            .unwrap();
        conn.execute("DELETE FROM workspaces WHERE id = 'ws-eta-test'", ())
            .await
            .unwrap();
    }
}
