...
use crate::database;
use crate::{
    Assignment, AttendanceRecord, Course, HealthIncident, HealthRecord, LibraryBook, ReportCard,
    SchoolInvoice, StudentProfile, Submission, TermGrade, TimetableSlot, YntraError,
};
...
async fn test_school_service_crud() {
    ...
    save_student_profile("u-school-admin".to_string(), profile.clone(), None)
        .await
        .unwrap();

    let list = get_student_profiles("u-school-admin".to_string(), "ws-school-test".to_string())
        .await
        .unwrap();
    ...
    save_library_book("u-school-admin".to_string(), book.clone(), None)
        .await
        .unwrap();

    checkout_book(
        "u-school-admin".to_string(),
        "ws-school-test".to_string(),
        "bk-1".to_string(),
        "stud-1".to_string(),
    ...
    .await
    .unwrap();
    let bk_list = get_library_books("u-school-admin".to_string(), "ws-school-test".to_string())
        .await
        .unwrap();
    ...
    create_school_invoice("u-school-admin".to_string(), invoice, None)
        .await
        .unwrap();
    ...
    let invoices = get_school_invoices("u-school-admin".to_string(), "ws-school-test".to_string())
        .await
        .unwrap();
    ...
    link_parent_to_student(
        ...
        "ws-school-test".to_string(),
        "stud-1".to_string(),
        "u-parent-1".to_string(),
        None,
    )
    .await
    .unwrap();
    let parents = get_student_parents(
        "u-school-admin".to_string(),
        "ws-school-test".to_string(),
        "stud-1".to_string(),
    )
    ...
    save_student_health_record("u-school-admin".to_string(), hr, None)
        .await
        .unwrap();
    let hr_list = get_student_health_records(
        "u-school-admin".to_string(),
        "ws-school-test".to_string(),
        "stud-1".to_string(),
    )
    ...
    save_health_incident("u-school-admin".to_string(), incident, None)
        .await
        .unwrap();
    let inc_list = get_health_incidents("u-school-admin".to_string(), "ws-school-test".to_string())
        .await
        .unwrap();
    ...
    save_term_grade("u-school-admin".to_string(), tg, None)
        .await
        .unwrap();
    let tg_list = get_term_grades(
        "u-school-admin".to_string(),
        "ws-school-test".to_string(),
        "stud-1".to_string(),
    )
    ...
    let c_tg_list = get_course_term_grades(
        "u-school-admin".to_string(),
        "ws-school-test".to_string(),
        "crs-1".to_string(),
    )
    ...
    save_assignment("u-school-admin".to_string(), test_assignment, None)
        .await
        .unwrap();
    ...
    save_submission("u-school-admin".to_string(), sub.clone(), None)
        .await
        .unwrap();
    let sub_list = get_student_submissions(
        "u-school-admin".to_string(),
        "ws-school-test".to_string(),
        "stud-1".to_string(),
    )
    ...
    save_timetable_slot("u-school-admin".to_string(), slot.clone(), None)
        .await
        .unwrap();
    let slots = get_timetable_slots("u-school-admin".to_string(), "ws-school-test".to_string())
        .await
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM submissions WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM timetable_slots WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM assignments WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM report_cards WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM term_grades WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM courses WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM health_incidents WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM health_records WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM student_parents WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM school_payments WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM school_invoices WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM library_lending_logs WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM library_books WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM student_profiles WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM users WHERE workspace_id = 'ws-school-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-school-test'", ())
        .await
        .unwrap();
}
...
async fn test_school_permission_validation() {
    ...
    let save_res = save_course(
        "u-teacher".to_string(),
        "ws-perm-test".to_string(),
        course.clone(),
        None,
    )
    .await;
    ...
    let save_student_res = save_course(
        "u-student".to_string(),
        "ws-perm-test".to_string(),
        course.clone(),
        None,
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM courses WHERE workspace_id = 'ws-perm-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE workspace_id = 'ws-perm-test'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-perm-test'", ())
        .await
        .unwrap();
}
...
async fn test_grading_conflict_resolution() {
    ...
    save_term_grade("u-admin-g".to_string(), grade_v2_a, None)
        .await
        .unwrap();
    crate::infra::time::sleep_ms(10).await;

    // Fetch updated_at after A's write
    let _saved_time_b: i64 = conn
        .query_row(
            "SELECT updated_at FROM term_grades WHERE id = 'tg-g'",
            (),
            |r| r.get(0),
    ...
    save_term_grade("u-admin-g".to_string(), grade_v2_b, None)
        .await
        .unwrap();

    // 4. Assert conflict encoding
    let (final_grade, final_points, teacher_comments): (String, Option<i64>, String) = conn
        .query_row(
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM term_grades WHERE workspace_id = ?1",
        crate::params![ws_id],
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM courses WHERE workspace_id = ?1",
        crate::params![ws_id],
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM student_profiles WHERE workspace_id = ?1",
        crate::params![ws_id],
    )
    .await
    .unwrap();
...
async fn test_student_profile_conflict_resolution() {
    ...
    save_student_profile("u-admin-p".to_string(), profile_v1, None)
        .await
        .unwrap();
    crate::infra::time::sleep_ms(10).await;
    ...
    save_student_profile("u-admin-p".to_string(), profile_v2_a, None)
        .await
        .unwrap();
    crate::infra::time::sleep_ms(10).await;
    ...
    save_student_profile("u-admin-p".to_string(), profile_v2_b, None)
        .await
        .unwrap();
...
async fn test_attendance_record_conflict_resolution() {
    ...
    save_attendance_record("u-admin-a".to_string(), rec_v1, None)
        .await
        .unwrap();
    crate::infra::time::sleep_ms(10).await;
    ...
    save_attendance_record("u-admin-a".to_string(), rec_v2_a, None)
        .await
        .unwrap();
    crate::infra::time::sleep_ms(10).await;
    ...
    save_attendance_record("u-admin-a".to_string(), rec_v2_b, None)
        .await
        .unwrap();
    ...
    conn.execute(
        "DELETE FROM attendance_records WHERE workspace_id = ?1",
        crate::params![ws_id],
    )
    .await
    .unwrap();
...
async fn test_timetable_slot_conflict_resolution() {
    ...
    save_timetable_slot("u-admin-s".to_string(), slot_v1, None)
        .await
        .unwrap();
    crate::infra::time::sleep_ms(10).await;
    ...
    save_timetable_slot("u-admin-s".to_string(), slot_v2_a, None)
        .await
        .unwrap();
    crate::infra::time::sleep_ms(10).await;
    ...
    save_timetable_slot("u-admin-s".to_string(), slot_v2_b, None)
        .await
        .unwrap();
    ...
    conn.execute(
        "DELETE FROM timetable_slots WHERE workspace_id = ?1",
        crate::params![ws_id],
    )
    .await
    .unwrap();
...
async fn test_health_incident_conflict_resolution() {
    ...
    save_health_incident("u-admin-h".to_string(), inc_v1, None)
        .await
        .unwrap();
    crate::infra::time::sleep_ms(10).await;
    ...
    save_health_incident("u-admin-h".to_string(), inc_v2_a, None)
        .await
        .unwrap();
    crate::infra::time::sleep_ms(10).await;
    ...
    save_health_incident("u-admin-h".to_string(), inc_v2_b, None)
        .await
        .unwrap();
    ...
    conn.execute(
        "DELETE FROM health_incidents WHERE workspace_id = ?1",
        crate::params![ws_id],
    )
    .await
    .unwrap();
...
async fn test_school_row_level_sync() {
    ...
    conn.execute(
        "UPDATE student_profiles SET user_id = 'u-student-sync' WHERE id = 'stud-sync-1'",
        (),
    )
    .await
    .unwrap();
    ...
    crate::database::sync::configure_database_sync(
        "local_url".to_string(),
    ...
    // Clean up
    conn.execute(
        "DELETE FROM submissions WHERE workspace_id = ?1",
        crate::params![ws_id],
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM assignments WHERE workspace_id = ?1",
        crate::params![ws_id],
    )
    .await
    .unwrap();
...
async fn test_school_conflict_resolution() {
    ...
    let conflicts = get_school_conflicts("u-conflict-admin".to_string(), ws_id.to_string())
        .await
        .unwrap();
    ...
    let (resolved_status, resolved_notes): (String, Option<String>) = conn
        .query_row(
    ...
    let remaining = get_school_conflicts("u-conflict-admin".to_string(), ws_id.to_string())
        .await
        .unwrap();
