...
async fn test_vehicle_management_flow() {
    ...
    let v1 = create_vehicle(
        "u-veh-staff".to_string(),
        "Truck A".to_string(),
        "ABC-123".to_string(),
        45.5,
        None,
    ...
    delete_vehicle("u-veh-staff".to_string(), v2.id.clone())
        .await
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM vehicles WHERE workspace_id = 'ws-veh-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE id = 'u-veh-staff'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-veh-test'", ())
        .await
        .unwrap();
}
...
async fn test_vehicle_gps_tracking_and_simulation() {
    ...
    simulate_vehicle_movement("u-gps-staff".to_string(), v.id.clone(), 5)
        .await
        .unwrap();
    ...
    // Cleanup
    conn.execute(
        "DELETE FROM vehicles WHERE workspace_id = 'ws-gps-sim-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE id = 'u-gps-staff'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-gps-sim-test'", ())
        .await
        .unwrap();
}
...
async fn test_hardware_gps_webhook_processing() {
    ...
    let v = create_vehicle(
        "u-hw-gps-staff".to_string(),
        "HW Sim Truck".to_string(),
        "GPS-HW-888".to_string(),
        35.0,
        Some("IMEI-HW-999".to_string()),
    )
    .await
    .unwrap();
    ...
    register_gps_ping_from_webhook(
        "ws-hw-gps".to_string(),
    ...
    let payload_abax =
        r#"{"equipmentId": "IMEI-HW-999", "position": {"lat": 60.1699, "lon": 24.9384}}"#;
    register_gps_ping_from_webhook(
        "ws-hw-gps".to_string(),
        "secret-token-123".to_string(),
        payload_abax.to_string(),
    )
    .await
    .unwrap();
    ...
    let err_c = register_gps_ping_from_webhook(
        "ws-hw-gps".to_string(),
        "wrong-token".to_string(),
        payload_err.to_string(),
    )
    .await;
    ...
    let err_d = register_gps_ping_from_webhook(
        "ws-hw-gps".to_string(),
        "secret-token-123".to_string(),
        payload_d.to_string(),
    )
    ...
    // Cleanup
    conn.execute("DELETE FROM vehicles WHERE workspace_id = 'ws-hw-gps'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM users WHERE id = 'u-hw-gps-staff'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-hw-gps'", ())
        .await
        .unwrap();
}
...
async fn test_commercial_truck_navigation_clearance_rules() {
    ...
    let profile_heavy =
        get_vehicle_commercial_routing_profile("u-nav-staff".to_string(), v_heavy.id.clone())
            .await
            .unwrap();
    ...
    let profile_small =
        get_vehicle_commercial_routing_profile("u-nav-staff".to_string(), v_small.id.clone())
            .await
            .unwrap();
    ...
    conn.execute(
        "DELETE FROM vehicles WHERE workspace_id = 'ws-nav-rules'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE id = 'u-nav-staff'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-nav-rules'", ())
        .await
        .unwrap();
}
...
async fn test_realtime_gps_telemetry_streaming() {
    ...
    let v = create_vehicle(
        "u-stream-staff".to_string(),
        "Stream Truck".to_string(),
        "STR-100".to_string(),
        30.0,
        None,
    )
    ...
    let telemetry = get_active_vehicles_telemetry("u-stream-staff".to_string())
        .await
        .unwrap();
    ...
    conn.execute(
        "DELETE FROM vehicles WHERE workspace_id = 'ws-stream-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE id = 'u-stream-staff'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-stream-test'", ())
        .await
        .unwrap();
}
...
async fn test_eld_hos_and_dvir_dot_compliance_workflow() {
    ...
    let v = create_vehicle(
        ...
        "ELD Interstate Truck".to_string(),
        "ELD-888".to_string(),
        40.0,
        None,
    )
    ...
    assert!(
        hos_viol
            .violation_reason
            .unwrap()
            .contains("11-Hour Driving Limit")
    );
    ...
    let dvir_list = get_vehicle_dvir_reports("u-eld-staff".to_string(), v.id.clone())
        .await
        .unwrap();
    ...
    // Check Summary
    let summary = get_vehicle_dot_compliance_summary("u-eld-staff".to_string(), v.id.clone())
        .await
        .unwrap();
    ...
    conn.execute(
        "DELETE FROM eld_hos_logs WHERE workspace_id = 'ws-eld-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM dvir_inspections WHERE workspace_id = 'ws-eld-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM ifta_fuel_logs WHERE workspace_id = 'ws-eld-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute(
        "DELETE FROM vehicles WHERE workspace_id = 'ws-eld-test'",
        (),
    )
    .await
    .unwrap();
    conn.execute("DELETE FROM users WHERE id = 'u-eld-staff'", ())
        .await
        .unwrap();
    conn.execute("DELETE FROM workspaces WHERE id = 'ws-eld-test'", ())
        .await
        .unwrap();
}
