---
title: "Architecture Overview Tripartite Local-First System"
head:
  - tag: script
    attrs:
      src: /target-blank.js
...
3. **Reactive FFI Observers**: UI components subscribe to [`DatabaseObserver`](https://github.com/YntraAB/YntraPlatform/blob/main/yntra-core/src/infra/observer.rs#L5-L85) state notifications instead of polling.
4. **Zero-Copy Boundary**: Inter-process memory transfers use `rkyv` zero-copy serialization, avoiding JSON parsing overhead on hot execution paths.