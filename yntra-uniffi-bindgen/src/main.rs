...
fn main() {
    ...
    let run_generation = |release: bool| -> Result<(), String> {
        ...
        export_block_schemas(&workspace_root)?;
...
fn export_block_schemas(workspace_root: &Path) -> Result<(), String> {
    ...
    let content = fs::read_to_string(&blocks_rs)
        .map_err(|e| format!("Failed to read blocks.rs: {}", e))?;
    ...
    for line in content.lines() {
        ...
        if trimmed.starts_with("id: \"") {
            if let Some(end) = trimmed[5..].find('"') {
                current_block_id = trimmed[5..5 + end].to_string();
            }
        } else if trimmed.starts_with("name: \"") {
            if let Some(end) = trimmed[7..].find('"') {
                current_block_name = trimmed[7..7 + end].to_string();
                if !current_block_id.is_empty() {
                    block_items.push((current_block_id.clone(), current_block_name.clone()));
    ...
    let json_array: Vec<serde_json::Value> = block_items
        .iter()
        .map(|(id, name)| {
            serde_json::json!({
                "block_id": id,
                ...
                "ffi_schema_version": "1.0"
            })
        })
        .collect();

    let json_str = serde_json::to_string_pretty(&json_array).unwrap_or_default();
    fs::write(out_dir.join("block_schemas.json"), &json_str)
        .map_err(|e| format!("Failed to write block_schemas.json: {}", e))?;

    let swift_content = format!(
        "// Auto-generated by yntra-uniffi-bindgen\nimport Foundation\n\npublic struct BlockSchema: Codable {{\n    public let blockId: String\n    public let name: String\n    public let supportsDynamicUi: Bool\n}}\n\npublic class BlockSchemaRegistry {{\n    public static let shared = BlockSchemaRegistry()\n    public let blocks: [BlockSchema] = {{\n        let json = \"\"\"{}\"\"\"\n        return (try? JSONDecoder().decode([BlockSchema].self, from: Data(json.utf8))) ?? []\n    }}()\n}}\n",
        json_str.replace('\\', "\\\\").replace('"', "\\\"")
    );

    fs::write(out_dir.join("BlockSchemaRegistry.swift"), swift_content)
        .map_err(|e| format!("Failed to write BlockSchemaRegistry.swift: {}", e))?;
